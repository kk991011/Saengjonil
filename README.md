...
회원이 내 일지에서 직접 입력한 사용 항목, 금액, 사용 시각, 메모와 발급 건별 차감 배분(`allocations`)을 기록한다. 사용 시 만료일이 가까운 유효 쿠폰부터 `remainingAmount`가 차감되며 누적 발급 통계는 바뀌지 않는다. 관리자는 수령인의 쿠폰 상세에서 이 사용 내역을 확인한다.
