...
| `main.html` | 내 일지 | **본인의 생존 쿠폰 잔액·건별 만료일 조회 및 사용 항목·금액 직접 기록**, 오늘 기록(매십경·면·독·운 루틴 + 취준활동: **강의 수강[정해진 강의별 분 입력]**·자소서·필기·면접·지원), 대시보드(내 추이 + **조별 추이[내 조 선택기]**), 주차별 추이, 주간/월간 목표, 누적 요약, 프로필/테마 설정(+**이전 시즌 기록** 입력, 소속 조 읽기전용), 본인 재인증 후 회원 탈퇴 |
| `compare.html` | 함께 생존 | 탭 6개: **랭킹 / 매십경 상세 / 매십면 상세 / 항목 비교(+이전 시즌 컬럼·엑셀) / 강의 수강(강의별 기간 합산·엑셀) / 그룹별 현황**. 상단 **"내 조" 선택기**(다중 소속 시 노출, 그룹별 현황 탭에선 숨김) |
| `admin.html` | 관리자 | 전체 현황, **기간·조·프로그램별 MVP TOP 3/통계**, 그룹 CRUD, 참여자 **최근 7일 신규 가입·미배정 필터/가입일 확인**, 유저 **닉네임 변경**·조 다중 배정(체크박스)·프로그램 변경·온보딩 초기화·삭제, **그룹 수정에서 조장 지정**, 유저 상세, **생존 쿠폰 개별 발급·회원 사용 내역 확인·취소·만료 내역 조회/삭제·검색·누적 통계** |
...
| 취준활동 | `lecture` / `jasoseo` / `jasoseoCount` / `pilgi` / `interview` / `cert`(자격증 준비 분) / `totalTime` / `applications` (number), `lectureItems`(map: 강의명→분; `lecture`는 그 합계), `additionalActivities`(array of `{ name, minutes }`), `etc`(직접 추가 시간 합계·레거시 호환) |
...
> 구 기록의 `fa5050`은 현장방문 여부 필드입니다. `siteVisitCount`가 없는 기록은 `fa5050=true`를 현장방문 1회로 호환하며, FR5050 달성 여부에는 사용하지 않습니다.
> `additionalActivities`가 없는 구 기록은 `etc` 값을 이름이 `기타`인 직접 추가 항목으로 표시합니다.
...
### `coupon_stats` — 문서 ID = **쿠폰 수령인 ID**
| 필드 | 타입 | 설명 |
|---|---|---|
| `totalIssuedAmount` | number | 만료 후에도 유지되는 누적 발급 금액 |
| `totalIssueCount` | number | 만료 후에도 유지되는 누적 발급 횟수 |
...
회원이 내 일지에서 직접 입력한 사용 항목, 금액, 사용 시각, 메모와 발급 건별 차감 배분(`allocations`)을 기록한다. 사용 시 만료일이 가까운 유효 쿠폰부터 `remainingAmount`가 차감되며 누적 발급 통계는 바뀌지 않는다. 관리자는 수령인의 쿠폰 상세에서 사용 항목·메모를 수정하거나 잘못된 사용을 취소해 배분된 금액을 원래 발급 건으로 복구할 수 있다. 복구 시 발급일과 만료일은 변경하지 않는다.

### `coupon_history` — 문서 ID = **`coupon_issues`와 동일한 ID**

회원이 내 일지의 생존 쿠폰 탭에서 누적 발급 내역을 계속 확인할 수 있도록 보존하는 이력이다. 만료 후 `coupon_issues`를 삭제해도 이 문서는 남으며, 만료 전 발급 취소 시에는 누적 통계와 함께 삭제된다. 기존 발급 건은 관리자가 쿠폰 페이지를 처음 연 시점에 누락분을 자동 복사한다.

### `coupon_recipients` — 문서 ID = **자동 생성 ID**
| 필드 | 타입 | 설명 |
|---|---|---|
| `name` / `contact` / `note` | string | 생존일지 미가입 수령인의 이름, 선택 연락처, 관리자 메모 |
| `createdAt` / `createdBy` | timestamp / string | 등록 시각과 등록 관리자 UID |

미가입자는 `users`에 만들지 않으므로 전체 참여자 수, 그룹, 활동률, MVP 통계에 포함되지 않는다. 쿠폰 검색과 통계에서만 가입자와 합쳐 표시한다.
...
coupon_history.recipientId ─▶ 쿠폰 수령인        (누적 발급 이력 보존)
```
